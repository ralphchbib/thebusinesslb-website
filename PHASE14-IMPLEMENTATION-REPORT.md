...
- `payload/access-network.ts` — `isStaff` broadened to include `moderator` (needed so a moderator can read the existing content collections a case must render)
- `payload/collections/Users.ts` — added the `moderator` role option
- `payload/collections/ContentReports.ts` — added `case`/`contentSnapshot` fields and the find-or-create-case hook
