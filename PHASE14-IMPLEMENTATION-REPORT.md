...
*That review found the remediation genuinely fixed all three original findings for their primary threat scenarios, but — probing specifically for narrow correctness — found a real, live-reproduced gap in the escalation fix itself, plus a lower-severity residual gap in the appeal-duplicate fix and a live-observed (likely environment-specific) suspension-enforcement failure. §K documents the second-pass fix.*

## K. Remediation V2 (Post-Review-V2)

Full root-cause analysis in `PHASE14-REMEDIATION-V2-PLAN.md`, written before any fix was applied.

**1. `hasDecidedCaseHistory` counted `no-action` decisions as offense history (`PHASE14-RELEASE-REVIEW-V2.md` §C.1).** The function's `decision: { exists: true }` filter matched any decided case, including one cleared as `no-action` — meaning an account that was reported and cleared once permanently lost its first-offense escalation protection for every future case. Fixed by narrowing the filter to `decision: { in: ["content-removed", "warning-issued", "account-suspended"] }` (a new `SANCTION_DECISIONS` constant in `payload/access-moderation.ts`), matching the design's own §E framing of case history as "count of prior **action-taken** decisions." Confirmed live, in sequence: an account with one prior `no-action` case is still correctly treated as first-offense (a lone moderator's direct suspension attempt on a second, unrelated case is rejected); after that second case is instead decided `warning-issued` (a genuine sanction), a *third* case on the same account is correctly decidable directly by a lone moderator — confirming the fix recognizes real history without either under- or over-counting it.

**2. Staff-initiated appeal creation bypassed the duplicate check (`PHASE14-RELEASE-REVIEW-V2.md` §C.2, secondary hardening, not itself a merge blocker).** `createAppeal`'s early `if (isModerationStaff(user)) return true` returned before the existing-appeal query ever ran, so a staff-initiated create relied solely on the database's unique index rather than the access function's own stated guarantee. Restructured so the duplicate check runs unconditionally for every caller; only the ownership/deadline checks (which don't apply to staff — an appeal isn't staff's own to own a deadline against) remain staff-exempt. Confirmed live: a staff (admin) attempt to create a second appeal on an already-appealed case is now rejected by the access function itself, not just the database constraint.

**3. Suspension enforcement re-verified under normal conditions (`PHASE14-RELEASE-REVIEW-V2.md` §E.3).** The prior review observed 3/3 live failures of the secondary `network-accounts.status` write, attributing it to possible sandbox network latency rather than a code defect, and asked for re-verification. Re-ran five full suspend → verify-blocked → reactivate → verify-restored cycles this pass: **5/5 succeeded** — `status` flipped to `suspended` every time, login was blocked every time, and login succeeded again after reactivation every time (decision writes took ~12–13 seconds each in this environment, slow but not a failure). This confirms the prior observation was **(B) an environment/network-latency artifact, not (A) a real defect** — no code in the suspension-enforcement path was touched by this remediation pass; the mechanism is exactly what it was when review V1 also confirmed it working.

**Validation re-run after this pass:** `npx tsc --noEmit` (clean), `npm run lint` (clean), `npm test` (4/4 pass), `npm run build` (clean, 59 routes — three attempts hit the same pre-existing, unrelated Supabase pooler `ENOTFOUND tenant/user postgres.zuclv not found` transient error already documented in `PHASE14-RELEASE-REVIEW-V2.md` §B/§E.4, each time on a different unrelated pre-existing page (`/about/how-we-work`, `/insights/[slug]`, `/contact`), never on anything this phase touches; a clean run followed each time on retry). Live re-verification via a third temporary, uncommitted test harness (same pattern as prior passes, deleted before commit). All test data deleted after validation; final inventory confirmed zero rows remaining.

**Recommendation:** a third independent release review, per instruction — see `PHASE14-RELEASE-REVIEW-V3.md`.
