...
- A network account can create at most one profile of its own account type, and only of its own account type — enforced at the `access.create` layer itself (`createBusinessProfile`/`createProfessionalProfile` in `payload/access-profiles.ts`, added after PR #17's independent release review found the original `create: createByNetworkAccount` didn't check `accountType` or existing-profile count, and confirmed live that a Consumer account could create a business profile via direct API). This is access-control-enforced, not just a Server Action convenience, though it is still not a hard database uniqueness constraint (Payload has no compound-unique/one-per-relationship constraint) — an async access function queries for an existing profile before allowing create, closing the gap at the actual enforcement point rather than only in the UI path.
